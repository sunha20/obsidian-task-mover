...
export default class UndoModal extends Modal {
    ...
    async onOpen() {
        ...
        contentEl.createEl('div', {text: undoHistoryInstance.previousDay.length});

        // previousDay가 배열이므로 각 요소를 순회하여 처리

        if (undoHistoryInstance.previousDay.length > 0) {
            for (const file of undoHistoryInstance.previousDay) {
                modTextArray.push(await this.parseDay(file));
            }
        }
